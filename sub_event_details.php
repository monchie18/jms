...
if (isset($_POST['save_settings'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO contestants (fullname, subevent_id, contestant_ctr, rand_code) VALUES (:fullname, :subevent_id, :contestant_ctr, :rand_code)");
    
    // Looping through contestants
    for ($i = 1; $i <= 40; $i++) {
        $con_name = $_POST['con' . $i];
        $rand_code = $_POST['rand' . $i];
        
        if (!empty($con_name)) {
            $stmt->bindParam(':fullname', $con_name);
            $stmt->bindParam(':subevent_id', $sub_event_id);
            $stmt->bindParam(':contestant_ctr', $i);
            $stmt->bindParam(':rand_code', $rand_code);
