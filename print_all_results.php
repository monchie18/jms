                ...
                ?>
                        ...
                        <table class="table table-bordered">
                            <thead>
                                ...
                                <th>Placing</th> <!-- Added Placing Column -->
                            </thead>
                            <tbody>

                                <?php
                                $ranking = 1; // Initialize ranking variable
                                // Modified SQL query to sort contestants by average score
                                $o_result_query = $conn->query("SELECT DISTINCT contestant_id, AVG(total_score) as avg_score FROM sub_results WHERE mainevent_id='$active_main_event' AND subevent_id='$active_sub_event' GROUP BY contestant_id ORDER BY avg_score DESC") or die(mysql_error());
                                while ($o_result_row = $o_result_query->fetch()) {

                                    $contestant_id = $o_result_row['contestant_id'];
                                    $avg_score = $o_result_row['avg_score'];
                                ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php echo "Average Score: " . round($avg_score, 1); ?>
                                        </td>
                                        <td>
                                            <center>
                                ...
                                <?php
                                    $ranking++; // Increment the ranking for the next loop iteration
                                }
                                ?>
